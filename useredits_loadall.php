...
if(isset($_POST['oper']) && $_POST['oper'] == 'umrk'){
  for ($ni=0;$ni<$_POST['logcount'];$ni++) {
    if(isset($_POST['logid'.$ni])){
      $logid = $_POST['logid'.$ni];
...
if(isset($_POST['oper']) && $_POST['oper'] == 'mrk'){
  for ($ni=0;$ni<$_POST['logcount'];$ni++) {
    if(isset($_POST['logid'.$ni])){
      $logid = $_POST['logid'.$ni];
      $loguid = $_POST['loguid'.$ni];
...
if(isset($_POST['oper']) && $_POST['oper'] == 'dlv'){
  $sql = 'delete FROM editlogsviewed WHERE logid in (select logid from editlogs where whatsnew = 0 and flagged = 0 and editdate < DATE_ADD(UTC_TIMESTAMP(),INTERVAL -200 DAY)) ';
  $mrk = dbquery($sql);
}
...
case 2: // unread
  $sql = 'select count(*)
          from editlogs
          where editdate > DATE_ADD(UTC_TIMESTAMP(),INTERVAL -'.$daysback.' DAY)'.$noednotes.$noblog.'
...
$sql = 'select count(*)
        from editlogs
        where userid != '.$userid.'
        and editdate > DATE_ADD(UTC_TIMESTAMP(),INTERVAL -1 DAY)'.$noednotes.$noblog.'
        and logid not in (select logid from editlogsviewed where userid = '.$userid.')
        limit 1';
*/
$sql = 'select count(*)
...
if(1==2 && $userid==1){
  $sql = 'select count(*)
          from editlogsviewed ';
  $row = rs($sql);
  $edlogsviewed = '<br />'.$row[0].' rows in the editlogsviewed table. <input type="button" value="clean" onclick="if(validate(document.frm, \'delv\')) document.frm.submit();" />';
}else $edlogsviewed = '';
...
<table style="border:0;">
  <tr><td style="font-size:90%;">&nbsp;<br /><span style="color:red;"><span style="font-size:120%"><?=$etot?></span> unread and/or flagged edit(s).</span><br /><?=$ttot?> total edits in system.<?=$edlogsviewed?></td></tr>
  <tr><td>
<small>For the past:
...
case 2: // unread
  $sql = 'select userid, ifnull(revusername, myrevname), if(userid=2, 1, 2)
          from myrevusers
          where myrevid > 0 and userid in (select distinct e.userid from editlogs e where e.editdate > DATE_ADD(UTC_TIMESTAMP(),INTERVAL -'.$daysback.' DAY)'.$noednotes.$noblog.'
...
while ($row = mysqli_fetch_array($usrs)) {
  ...
  $sql = 'select ifnull(max(editdate), 0)
          from editlogs
          where userid = '.$uid.$noednotes.$noblog.'
  ...
  print('<tr><td><a class="comlink0" onclick="showtbl('.$row[0].')"><img id="img'.$row[0].'" src="/i/tbl_show.png" alt="" border="0" style="width:'.((!$ismobile)?'1.0em':'.9em').';" title="expand" />&nbsp;<b>'.$row[1].'</b></a>&nbsp;Edits: '.$tot.'; Last: '.rtrim(date('n/j g:ia', strtotime(converttouserdate($last, $timezone))), 'm').'</td></tr>');
  print('<tr><td><div id="edts'.$row[0].'" style="height:0;overflow:hidden;transition:height .6s;text-align:left;"><table class="gridtable" style="width:700px;">');
  ...
  print('</tr>');
  if($fireexpcoldiv==0 && $uid==$expuid && $exp==1) $fireexpcoldiv = $uid;
  ...
  $sql = 'select b.title, b.abbr, e.page, e.testament, e.book, e.chapter, e.verse, e.editdate, e.logid, ifnull(elv.userid, 0) logviewed, ifnull(elv.flagged, 0) flagged,
          ...
          left join verse v on (v.testament = e.testament and v.book = e.book and v.chapter = e.chapter and v.verse = e.verse)
          left join editlogsviewed elv on (elv.userid = '.$userid.' and elv.loguserid = '.$uid.' and elv.logid = e.logid)
          where e.userid = '.$uid.$noednotes.$noblog.'
...
if($ni==0) print('<table><tr><td colspan="8" style="color:red;">&nbsp;<br /><small>No edits meet your filtering criteria.</small></td></tr></table>');

?>
</table>
<input type="hidden" name="logcount" value="<?=$ni?>">
<small>With checked:</small>
<?if($ni>0 && $superman==1){?>
<input type="submit" name="btnx" value="Delete" onclick="return validate(document.frm, 'del');">
<?}
if($ni>0){?>
&nbsp;&nbsp;<small>Mark as:</small>
<input type="submit" name="btny" value="Unread" onclick="return validate(document.frm, 'umrk');">
<input type="submit" name="btnz" value="Read" onclick="return validate(document.frm, 'mrk');">
<?}?>
<!--<br />
...
<?if($fireexpcoldiv>0) print('setTimeout(\'showtbl('.$fireexpcoldiv.')\', 300);');?>
...
function fixedit($r){
  ...
  case 8: // appx/intro/wordstudy
    ...
    case 3: // appx
      ...
      break;
    case 4: // wordstudy
      $ret = 'wordstudy: <a href="/wordstudy/'.$r['title'].'/1"'.$str.'>'.$r['title'].'</a>';
      break;
    };
    break;
  case 27:
