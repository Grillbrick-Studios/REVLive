...
var findcom = {};
findcom.parseReference = function (textReference) {
  ...
  if (possibleMatch != null) {
    ...
    else if(possibleMatch.indexOf('commentaries on ')==0)
      possibleMatch = possibleMatch.slice(16);
    /*
    else if(possibleMatch.indexOf('note on ')==0)
      possibleMatch = possibleMatch.slice(8);
    else if(possibleMatch.indexOf('notes on ')==0)
      possibleMatch = possibleMatch.slice(9);
    else if(possibleMatch.indexOf('entry on ')==0)
      possibleMatch = possibleMatch.slice(9);
    */
    ...
    if (bookIndex > -1) {
      params = findcom.parseScriptureRef(input, bookIndex);
      chapter1 = params.c1;
      chapter2 = params.c2;
      verse1   = params.v1;
      verse2   = params.v2;
  ...
  // finalize
  return findcom.Reference(bookIndex, chapter1, verse1, chapter2, verse2);

}

findcom.parseScriptureRef = function(txt, bidx){
...
findcom.Reference = function () {
  ...
  if (args.length == 0) {
    ...
    return findcom.parseReference(args[0]);
  ...
  return {
    ...
    chapterAndVerse: function (cvSeparator, vvSeparator, ccSeparator) {
      ...
      // some of these are not necessary for findcomm
...
(function() {
  ...
  var
    ...
    getBibleText = function(refString, callback) { // rsw
      var reference = new findcom.Reference(refString);
      href = (reference.bookIndex+1) + ','
            ...
            + reference.verse2;
      //alert(href);
      //jsonp('http://' + findcomm.remoteURL + '/jsonverse.php?ref=' + encodeURIComponent(href), callback);
      jsonp(findcomm.remoteURL + '/jsonverse.php?ref=' + encodeURIComponent(href), callback);
    },
  ...
  var
    createFindCommLinks = function(newNode, reference) {
        ...
        newNode.setAttribute('class', findcomm.className);
    ...
    handleLinkMouseOver = function(e) {
      if (!e) {e = win.event;}
      if (!findcomm.enablePopups) {return;}
  ...
  var findcomm = {
      ...
      handleLinks: createFindCommLinks
    },
    win = window,
    doc = document,
    body = null,
   ...
   ver2 = '(?:\\d{1,3}(?::\\d+))(?:-\\d+(?::\\d+)?)?',
   ver3 = '(?: ?, ?'+ver+')|(?: ?(?:(?:(?:;|,)? and|;)) ?'+ver2+')',
   regexPattern =  '\\b(?:(?:commentary|commentaries) on )(?:'+bok+')\\.? '
          + '(?:' + ver
          + '(?:' + ver3 + ')*'
    ...
    createLinksFromNode = function(node, referenceNode) {
      ...
      if (typeof reference != 'undefined' && reference.isValid()) {
        ...
        newNode = node.ownerDocument.createElement(findcomm.newTagName);
        node.parentNode.replaceChild(newNode, referenceNode);

        // this can be overridden for other systems
        findcomm.handleLinks(newNode, reference);
    ...
    parseRefText = function(refText) {
      ...
      var
        text = refText,
        reference = new findcom.Reference(text),
        match = null,
        p1, p2, p3, p4;
      ...
      if (reference != null && typeof reference.isValid != 'undefined' && reference.isValid()) {
        ...
        if (match) {
          ...
          params = findcom.parseScriptureRef(input, lastReference.bookIndex);
          p1 = params.c1;
          p2 = params.v1;
          p3 = params.c2;
          p4 = params.v2;
    ...
    traverseDOM = function(node, depth, textHandler) {
      var count = 0,
        ...
        skipRegex = new RegExp('^(' + alwaysSkipTags.concat(findcomm.ignoreTags).join('|') + ')$', 'i');


      while (node && depth > 0) {
        count++;
        if (count >= findcomm.maxNodes) {
        ...
        switch (node.nodeType) {
          case 1: // ELEMENT_NODE
            if (!skipRegex.test(node.tagName.toLowerCase()) && node.childNodes.length > 0 && (findcomm.ignoreClassName == '' || node.className.toString().indexOf(findcomm.ignoreClassName) == -1)) {
    ...
    scan=function() {
      ...
      doc = document;
      body = doc.body;
      if (findcomm.startNodeId != '') {
        node = doc.getElementById(findcomm.startNodeId);
      }
  ...
  //addEvent(doc,'DOMContentLoaded',startFindcomm);
  //addEvent(win,'load',startFindcomm);

  if (typeof window.findcomm != 'undefined')
    findcomm = extend(findcomm, window.findcomm);

  // export
  findcomm.scan = scan;
  findcomm.scanForReferences = scanForReferences;
  win.findcomm = findcomm;
})();
