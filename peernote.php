...
if(empty($userid) || $userid==0 || $peernotes==0) die('unauthorized access');
...
if($oper=='sav' || $oper=='del'){
  if($isworkspc==1){
    // dealing with reviewer workspace
    ...
    $beforenote = '';
    $beforedtls = '';
    $row = rs('select ifnull(peerworknotes, \'\') editdetails
              from myrevusers
    ...
    $sql = 'update myrevusers set
            peerworknotes = '.$editdetails.'
            where myrevid = '.$myrevid.' ';
    if(!is_null($dtlsdiff)){
      $update = dbquery($sql);
      $logid = logedit(310,0,0,$chap,$vers,$userid,'Reviewer workspace updated', 0, '', $dtlsdiff, 'null', 'null');
    ...
    // dealing with reviewer marginnotes and details
    ...
    if($oper=='del' || $row){
      if($oper=='del' || ($editdetails=='null' && $editnote=='null')){
        $sql = 'delete from peernotes
                where testament = '.$test.'
                and book = '.$book.'
                and chapter = '.$chap.'
                and verse = '.$vers.' ';
        $logid = logedit(310,$test,$book,$chap,$vers,$userid,'!Reviewer note deleted!', 0, $notediff, $dtlsdiff, 'null', 'null');
      }else{
        $sql = 'update peernotes set
                lastauthor = '.$userid.',
        ...
        if($notediff!='' || $dtlsdiff!='')
          $logid = logedit(310,$test,$book,$chap,$vers,$userid,'See changes', 0, $notediff, $dtlsdiff, 'null', 'null');

      }
    }else{
      if($editdetails != 'null' || $editnote != 'null'){
        $sql = 'insert into peernotes (testament, book, chapter, verse, author, lastauthor, notedate, lastupdate, resolved, colorindex, editnote, editdetails) values ('.
                $test.', '.$book.', '.$chap.', '.$vers.', '.$userid.', '.$userid.', UTC_TIMESTAMP, UTC_TIMESTAMP,'.$resolved.','.$coloridx.', '.$editnote.','.$editdetails.') ';

        // edit tracking
        $logid = logedit(310,$test,$book,$chap,$vers,$userid,'New reviewer note', 0, $notediff, $dtlsdiff, 'null', 'null');
...
if($isworkspc==1){
  $stitle = 'Reviewer Workspace';
  $sql = 'select ifnull(userid, 0) author,
                 ifnull(userid, 0) lastauthor,
                 0 resolved,
                 lastaccessed notedate, lastaccessed lastupdate,
                 ifnull(peerworknotes, \'\') editdetails,
                 \'\' editnote,
                 \'This is your Reviewer Workspace.\' versetext
          from myrevusers
          where myrevid = '.$myrevid.' ';

}else{
  $btitle = getbooktitle($test,$book, (($screenwidth>=480)?0:1));
  $stitle = (($ismobile)?'Rv Note: ':'Reviewer Notes on ').$btitle.(($test<2)?' '.$chap.':'.$vers:'');
  $sql = 'select notedate, lastupdate, ifnull(pn.author,-1) author,
                 ifnull(pn.lastauthor, -1) lastauthor,
                 ifnull(pn.resolved, 0) resolved,
                 ifnull(pn.editdetails, \'\') editdetails,
                 ifnull(pn.editnote, \'\') editnote,
                 if(v.versetext=\'-\', v.commentary, v.versetext) versetext,
                 v.footnotes
          from verse v
          left join peernotes pn on v.testament = pn.testament and v.book = pn.book and v.chapter = pn.chapter and v.verse = pn.verse
...
if($author==$userid || $author==-1){
  $mnlocked=0;
}else{
  $mnlocked = 1;
  $rrw = rs('select ifnull(revusername, myrevname) from myrevusers where myrevid > 0 and userid = '.$author.' ');
  $edusername = $rrw[0];
}
if($timessubmitted==0)
  logview((($isworkspc==1)?311:310),$test,$book,$chap,$vers);
...
<html>
<head>
  <title>Reviewer Note Editor</title>
...
<form name="frm" method="post" action="/peernote.php" style="padding:0;margin:0;">
...
<?
    ...
    if($isworkspc==1){
      ...
      if($mnlocked==1)
        print('<br /><small>Margin note (<span style="color:red;">This note was initiated by '.$edusername.'. You cannot edit the margin note.</span>):</small>
               <div class="peernote" style="margin-left:0;">'.$editnote.'</div>
...
<div style="position:absolute;bottom:0;left:0;right:0;height:50px;">
  ...
  <p style="text-align:center;margin:9px 0;">
    <?if($isworkspc==0 && ($author==$userid || ($author>0 && $superman==1))){
    ...
    <?if($isworkspc==0 && $superman==1 && $author>0){  // only SU can delete
...
<?}?>
  ...
  <script>
     ...
     function valdel(f){
       if(confirm('\nYou are about to completely remove the reviewer notes for this verse.\nIt cannot be undone.\n\nAre you sure you want to do this?')){
