...
if(!isset($page) || $editorcomments==0){
...
if($filter!==''){
  if(left($filter, 1)=='='){
    ...
    $filterstr.= 'and (v.versetext like \'%'.$filter.'%\' or en.editnote like \'%'.$filter.'%\' or en.editdetails like \'%'.$filter.'%\') ';
    $filter='='.$filter;
  }else{
    ...
    for($ni=0;$ni<sizeof($parts);$ni++){
      if(strlen($parts[$ni]) > 1){
        $filterstr.= 'and (v.versetext like \'%'.$parts[$ni].'%\' or en.editnote like \'%'.$parts[$ni].'%\' or en.editdetails like \'%'.$parts[$ni].'%\') ';
      }else{
...
$row = rs('select ifnull(notes, \'\') from myrevusers where myrevid = -1 ');
if($row && strlen($row[0])>0) $wsdot=1;
else $wsdot=0;

$resolved = ((isset($_POST['resolved']))?$_POST['resolved']:0);

// get list of people with editorcomment permission
$dat = dbquery('select ifnull(revusername, myrevname) username, ifnull(permissions, \'0:0:0:0:0:0\') permissions, if(userid=2, 1, 2) from myrevusers where userid > 0 order by 3,1 ');
$edusers = '';
$lastuser= '';
while ($row = mysqli_fetch_array($dat)) {
  $perms = explode(':', $row['permissions']);
  if($perms[5] == 1){
    $edusers.= (($lastuser == '')?'':$lastuser.', ');
    $lastuser = $row['username'];
  }
}
$edusers.= 'and '.$lastuser;

?>
<span class="pageheader" style="margin-bottom:3px;">REV Editor Notes
<a onclick="rlightbox('enote','-1|0|0|0|0',1);" title="Editor notes workspace"><img src="/i/editor_workspace<?=$colors[0].(($wsdot==1)?'_REDDOT':'')?>.png" alt="" style="width:1.8em;margin:0 0 -8px 0"></a>
...
<div style="width:100%;max-width:720px;text-align:center;padding:0;margin:0 auto;font-size:90%;">
  ...
  <div id="resinst" style="text-align:left;height:0;padding:3px;margin:0;overflow:hidden;transition:height .4s ease-in;">
    <h3 style="text-align:center;">Welcome to Editor Notes!</h3>
    <img src="/i/underconstruction.png" alt="under construction" style="border:0;width:104px;float:left;">
    <p>This page is where Editor Notes are managed. Editor Notes are comments about a scripture or commentary entry that are only visible to other editors.</p>
    ...
    <p>There are several Editors for the REV (<?=$edusers?>) and each one has the same permissions and abilities. Each editor is to operate within an honor system and to respect all margin and editor notes in the REV. For the editor note workspace, do not edit/delete any content that is not yours. Simply add whatever additional information you desire below what is already there.</p>
    <p style="margin-bottom:0;">When the issue raised in an editor note has been properly addressed, check the &ldquo;Resolved&rdquo; box at the bottom and then click &ldquo;Save.&rdquo; This will indicate to the other editors that the specific question(s) or concern(s) on that verse have been resolved.</p>
  </div>
</div>
...
switch($bookfilter){
case 0:
  $bookfilterstr='and en.testament in (0,1,3,4) ';break;
case -1:
  $bookfilterstr='and en.testament in (0,1) ';break;
case -2:
  $bookfilterstr='and en.testament in (0,1) and en.book between 1 and 39 ';break;
case -3:
  $bookfilterstr='and en.testament in (0,1) and en.book between 40 and 66 ';break;
case -4:
  $bookfilterstr='and en.testament in (0,1) and en.book between 40 and 43 ';break;
case -5:
  $bookfilterstr='and en.testament in (0,1) and en.book between 45 and 53 ';break;
case -6:
  $bookfilterstr='and en.testament = 3 ';break;
case -7:
  $bookfilterstr='and en.testament = 4 ';break;
default:
  $bookfilterstr='and en.testament in (0,1) and en.book = '.$bookfilter.' ';break;
}
switch($resolved){
case 0:
  $resolvedfilter='and en.resolved = 0 ';break;
case 1:
  $resolvedfilter='and en.resolved = 1 ';break;
default:
...
$comimg = '<img src="/i/commentary'.$colors[0].'.png" style="width:'.(($ismobile)?'1':'.8').'rem;" />';
  ...
  $sql = 'select count(*) from editnotes en
          join verse v on v.testament = en.testament and v.book = en.book and v.chapter = en.chapter and v.verse = en.verse
          where '.$where.' ';
  $row = rs($sql);
  $rsss= $row[0];
  ...
  $sql = 'select 1 from editnotes limit 1';
  $row = rs($sql);
  ...
  switch($sort){
  case 1: //entrydate
    $sortstr = 'en.notedate'.$ascdesc.' ';
    break;
  case 2: //lastupdate
    $sortstr = 'en.lastupdate'.$ascdesc.' ';
    break;
  default: //canon
    $sortstr = 'en.testament'.$ascdesc.', en.book'.$ascdesc.', en.chapter'.$ascdesc.', en.verse'.$ascdesc.' ';
    break;
  }
  $sql = 'select b.title, b.abbr, en.testament, en.book, en.chapter, en.verse, en.notedate, en.lastupdate,
          ...
          from editnotes en
          join book b on b.testament = en.testament and b.book = en.book
          join verse v on v.testament = en.testament and v.book = en.book and v.chapter = en.chapter and v.verse = en.verse
          left join myrevusers mr on (mr.userid = en.author and mr.myrevid > 0)
          left join myrevusers mr2 on (mr2.userid = en.lastauthor and mr2.myrevid > 0)
          where '.$where.'
  ...
  while ($row = mysqli_fetch_array($dat)) {
    ...
    if($row['editnote']!=''){
      ...
      $editnote = '<div id="bn_'.$nqry.'" class="editnote enotedarr" style="cursor:pointer;" onclick="rlightbox(\'enote\',\''.$nqry.'\',1);">'.$editnote.'</div>';
    }else{
      $editnote = '<div id="bn_'.$nqry.'" class="editnote enotedarr" style="display:none;cursor:pointer;" onclick="rlightbox(\'enote\',\''.$nqry.'\',1);"></div>';
    }
    $edtlink = '<a onclick="rlightbox(\'enote\',\''.$nqry.'\');" title="edit"><img src="/i/editor_notes'.$colors[0].((strlen($notes??'')>0)?'_REDDOT':'').'.png" style="width:2em;float:left;margin-right:4px;" alt="edit" /></a> ';
    if(strlen($notes??'') > $notestruncatelen){
      $readmore = '<a onclick="rlightbox(\'enote\',\''.$nqry.'\');" title="read more"> ..more</a> ';
  ...
  if($ni>0){
    ...
    if($havedata==0)
      print('There are no editor notes.');
    else
  ...
  function reloadeditnotes(qry){
    ...
    xmlhttp.onreadystatechange=function(){
      if (xmlhttp.readyState==4 && xmlhttp.status==200){
        ...
        var detl = ret.editdetails;
        var edtlink = '<a onclick="rlightbox(\'enote\',\''+qry+'\');" title="edit"><img src="/i/editor_notes'+colors[0]+((detl.length>0)?'_REDDOT':'')+'.png" style="width:2em;float:left;margin-right:4px;" alt="edit" /></a>';
        // this sorta works
